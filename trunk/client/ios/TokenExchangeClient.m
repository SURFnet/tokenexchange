...
static TokenExchangeClient *sharedInstance = nil;

@implementation TokenExchangeClient
...
- (void)setNotificationToken:(NSString *)notificationToken {
	...
	[defaults setValue:notificationToken forKey:@"TENotificationToken"];
	[defaults synchronize];
}

- (NSString *)notificationToken {
	NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
	return [defaults stringForKey:@"TENotificationToken"];
}
...
+ (TokenExchangeClient *)sharedInstance {
    @synchronized(self) {
        if (sharedInstance == nil) {
			sharedInstance = [[TokenExchangeClient alloc] init];
		}
	}
